...
async function main() {
  ...
  console.log('🚀 Starting complete deployment process with account:', deployer.address);
  console.log('Account balance:', ethers.formatEther(await deployer.provider.getBalance(deployer.address)), 'ETH');

  try {
    // Step 1: Deploy contracts using Ignition
    console.log('\n📦 Step 1: Deploying contracts with Ignition...');
    const { stdout: ignitionOutput, stderr: ignitionError } = await execAsync('npx hardhat ignition deploy ignition/modules/BettingPoolModule.ts');
    ...
    const { stdout: extractOutput, stderr: extractError } = await execAsync('npx hardhat run scripts/extract-addresses.ts');
    ...
    const { stdout: postDeployOutput, stderr: postDeployError } = await execAsync('npx hardhat run scripts/post-deployment.ts');
