...
async function main() {
  try {
    ...
    const artifactsPath = join(__dirname, '../ignition/deployments/chain-1337/artifacts');
    const files = readFileSync(artifactsPath, 'utf8').split('\n').filter(f => f.endsWith('.json'));
    ...
    const deploymentData: DeploymentArtifact = JSON.parse(readFileSync(deploymentPath, 'utf8'));
