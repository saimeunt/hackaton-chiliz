...
async function main() {
  ...
  try {
    ...
    console.log(
      '1. Update the contract addresses in the post-deployment script',
    );
    console.log(
      '2. Run: npx hardhat run scripts/post-deployment.ts --network <network>',
    );
    console.log('');
    console.log(
      'Contract addresses will be available in the Ignition deployment artifacts',
    );
  } catch (error) {
